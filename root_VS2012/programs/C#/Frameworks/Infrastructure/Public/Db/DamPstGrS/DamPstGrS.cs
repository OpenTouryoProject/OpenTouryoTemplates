...
{
    /// <summary>データアクセス・プロバイダ＝Npgsqlのデータアクセス制御クラス</summary>
    /// <remarks>必要なメソッド・プロパティを利用する</remarks>
    public class DamPstGrS : BaseDam
    {
        #region クラス変数

        /// <summary>パラメタの先頭記号（DBMSによって可変）</summary>
        /// <remarks>
        /// Npgsqlでは、パラメタの先頭記号に「@」・「:」の双方を使用可能だが、
        ...
        private NpgsqlDataAdapter _adpt;
        ...
        /// <summary>NpgsqlConnection（読み取り専用）</summary>
        /// <remarks>必要に応じて利用する。</remarks>
        public NpgsqlConnection DamNpgsqlConnection
        {
            get
            {
                // コネクションを戻す
        ...
        /// <summary>NpgsqlDataAdapter（読み取り専用）</summary>
        /// <remarks>
        /// 利用時、コネクション、トランザクションが有効な状態になっている事
        /// 必要に応じて利用する。
        /// </remarks>
        public NpgsqlDataAdapter DamNpgsqlDataAdapter
        {
            get
            {
                // コネクション、トランザクションを設定
                ...
                this._adpt = new NpgsqlDataAdapter(this._cmd);
        ...
        /// <summary>NpgsqlCommand（読み取り専用）</summary>
        /// <remarks>必要に応じて利用する。</remarks>
        public NpgsqlCommand DamNpgsqlCommand
        {
            get
            {
                // コマンドを戻す
        ...
        /// <summary>NpgsqlTransaction（読み取り専用）</summary>
        /// <remarks>必要に応じて利用する。</remarks>
        public NpgsqlTransaction DamNpgsqlTransaction
        {
            get
            {
                // トランザクションを戻す
        ...
        {
            // コネクションをオープン
            this._cnn = new NpgsqlConnection(connstring);
            this._cnn.Open();
        }
        ...
        /// <param name="parameterName">パラメタライズドクエリのパラメタ名</param>
        /// <param name="obj">パラメタの値</param>
        /// <param name="dbTypeInfo">パラメタの型（NpgsqlDbType）（設定しない場合は、nullを指定）</param>
        /// <remarks>通常、Ｄａｏ経由で利用する。</remarks>
            ...
            object dbTypeInfo, int size, ParameterDirection paramDirection)
        {
            ...
            else
            {
                ...
                if (dbTypeInfo != null)
                {
                    ...
                    {
                        // OFF扱い = NpgsqlDbType
                        param.NpgsqlDbType = (NpgsqlDbType)dbTypeInfo;
                    }
                }
        ...
        private void PreExecQuery()
        {
            if (this.IsDPQ)
            {
                ...
                this._cmd.CommandText = this.Convert(DamPstGrS._paramSign);
        ...
        /// <remarks>
        /// NpgsqlCommandのExecuteReaderを実行する。
        /// 通常、Ｄａｏ経由で利用する。
        /// </remarks>
        ...
        /// <remarks>
        /// NpgsqlCommandのExecuteScalarを実行する。
        /// 通常、Ｄａｏ経由で利用する。
        /// </remarks>
        ...
        /// <remarks>
        /// NpgsqlCommandのExecuteNonQueryを実行する。
        /// 通常、Ｄａｏ経由で利用する。
        /// </remarks>
        ...
        public override string GetCurrentQueryForLog()
        {
            ...
            else if (GetConfigParameter.GetConfigValue(PubLiteral.SQL_TRACELOG).ToUpper() == PubLiteral.ON)
            {
                ...
                // パラメタを取得
                foreach (NpgsqlParameter p in this._cmd.Parameters)
                {
